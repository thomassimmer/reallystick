...
use api::features::private_discussions::application::dto::requests::private_message::ListenForNewMessages;
...
async fn broadcast_ws(
    ...
    let token = match token_repo
        .get_by_user_and_token_id(request_claims.user_id, request_claims.jti)
        .await
    {
        Ok(Some(token)) => token,
        Ok(None) => {
