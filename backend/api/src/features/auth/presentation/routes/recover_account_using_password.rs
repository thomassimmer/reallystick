// Recover account using password route - uses clean architecture

use crate::core::constants::errors::AppError;
use crate::features::auth::application::dto::requests::RecoverAccountUsingPasswordRequest;
...
use crate::features::auth::application::use_cases::recover_account_using_password::RecoverAccountUsingPasswordUseCase;
...
pub async fn recover_account_using_password(
    req: HttpRequest,
    body: Json<RecoverAccountUsingPasswordRequest>,
    pool: Data<PgPool>,
    secret: Data<String>,
    ...
    let recover_account_use_case =
        RecoverAccountUsingPasswordUseCase::new(user_repo, recovery_code_repo, token_repo);
    let user = match recover_account_use_case
        .execute(
            body.username.clone(),
            body.password.clone(),
        ...
        .await
    {
        ...
        Err(AppError::InvalidUsernameOrPasswordOrRecoveryCode) => {
            if let Err(e) = transaction.rollback().await {
                error!("Error rolling back: {}", e);
            }
    ...
    let pool_clone_for_tokens = pool.get_ref().clone();
    let token_repo_for_tokens = UserTokenRepositoryImpl::new(pool_clone_for_tokens.clone());
