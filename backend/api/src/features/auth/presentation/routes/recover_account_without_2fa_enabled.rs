// Recover account without 2FA route - uses clean architecture

use crate::core::constants::errors::AppError;
use crate::features::auth::application::dto::requests::RecoverAccountWithout2FAEnabledRequest;
...
use crate::features::auth::application::use_cases::recover_account_without_2fa::RecoverAccountWithout2FAUseCase;
...
pub async fn recover_account_without_2fa_enabled(
    req: HttpRequest,
    body: Json<RecoverAccountWithout2FAEnabledRequest>,
    pool: Data<PgPool>,
    secret: Data<String>,
    ...
    let recover_account_use_case =
        RecoverAccountWithout2FAUseCase::new(user_repo, recovery_code_repo, token_repo);
        ...
        .await
    {
        ...
        Err(AppError::InvalidUsernameOrRecoveryCode) => {
            if let Err(e) = transaction.rollback().await {
                error!("Error rolling back: {}", e);
            }
