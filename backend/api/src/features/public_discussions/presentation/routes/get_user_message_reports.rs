...
pub async fn get_user_message_reports(
    ...
    let get_message_reports_result = report_repo
        .get_by_reporter_with_executor(request_claims.user_id, &mut *transaction)
        .await;
    let get_messages_result = message_repo
        .get_reported_by_user_with_executor(request_claims.user_id, &mut *transaction)
        .await;
