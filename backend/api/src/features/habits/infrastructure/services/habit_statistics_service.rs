// Habit statistics service - handles complex statistics queries
...
pub struct HabitStatisticsService {
    habit_repo: HabitRepositoryImpl,
    pool: PgPool,
}

impl HabitStatisticsService {
    pub fn new(pool: PgPool) -> Self {
        Self {
            habit_repo: HabitRepositoryImpl::new(pool.clone()),
            pool,
        }
    }

    pub async fn fetch_habit_statistics(&self) -> Result<Vec<HabitStatistics>, sqlx::Error> {
        ...
        let habits = self
            ...
            .await?;
        let habit_ids: Vec<Uuid> = habits.iter().map(|h| h.id).collect();
        ...
        for habit in habits {
            ...
            // Collect education statistics for the current habit
            ...
            // Collect urban area statistics for the current habit
