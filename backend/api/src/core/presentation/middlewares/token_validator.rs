    ...
    B: 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        let pool = req.app_data::<Data<PgPool>>().unwrap().get_ref().clone();
        ...
        Box::pin(async move {
            ...
            // Create repository
            let token_repo = UserTokenRepositoryImpl::new(pool);

            match TokenService::retrieve_claims_from_request_static(auth_header, secret.as_bytes())
            {
                Ok(claims) => {
                    ...
                    match last_activity_for_this_token {
                        ...
                        None => {
                            ...
                            let existing_token = token_repo
                                .get_by_user_and_token_id(claims.user_id, claims.jti)
                                .await;
